...
class Kriteria extends Model
{
    ...
    protected $table = 'import_kriteria';
    const tableName = 'import_kriteria';
    const excelFolder = 'assets/import/kriteria';
    ...
    {
        return $this->hasMany(ModelsKriteria::class, 'import_id', 'id');
    }
    ...
    public static function import(Request $request, Kriteria $model)
    {
        // setup
        ...
        foreach ($array_from_excel as $i => $v) {
            ...
            // cek
            $kode = $v[1];
            $cek = ModelsKriteria::where('kode', $kode)->count();
            if ($cek > 0) return [
                'status' => false,
                'error' => response()->json(['message' => "Kode $kode Sudah di gunakan, Silahkan cek data nomor {$v[0]}"], 400),
                'excel' => $file_excel
            ];
            $kriteria = new ModelsKriteria();
            $kriteria->kode = $kode;
            $kriteria->nama = $v[2];
            $kriteria->keterangan = $v[3];
            $kriteria->import_id = $model->id;
            $kriteria->save();
            $count++;
        }
    ...
    public static function format(Request $request)
    {
        ...
        $title_excel = "Formulir Import Kriteria";
        // Header excel ================================================================================================
        ...
        $sheet->mergeCells($col_start . $row . ":" . $col_end . $row)
            ->setCellValue("A$row", "Formulir Import Kriteria");
    ...
    {
        // data body
        $details = ModelsKriteria::orderBy('kode')->get();
        $bulan_array = [
            1 => 'Januari',
            2 => 'February',
            3 => 'Maret',
        ...
        $col_start = "A";
        $col_end = chr(64 + count($headers));
        $title_excel = "Export Semua Data Kriteria";
        // Header excel ================================================================================================
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $row += 1;
        $sheet->mergeCells($col_start . $row . ":" . $col_end . $row)
            ->setCellValue("A$row", "Export Semua Data Kriteria");
        $sheet->getStyle($col_start . $row . ":" . $col_end . $row)->applyFromArray([
            "font" => [
                "bold" => true,
                "size" => 13
